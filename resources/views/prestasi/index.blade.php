...
<div class="max-w-xl mx-auto px-0 sm:px-6 py-4 md:py-8 pb-24">
   ...
   <div class="mb-6 sm:rounded-xl overflow-hidden bg-white border sm:border border-gray-300 shadow-sm relative">
        ...
        <div class="h-32 md:h-44 bg-slate-700 relative overflow-hidden">
            <div class="absolute inset-0 bg-gradient-to-r from-slate-800 to-slate-600"></div>
            <img src="https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&q=80&w=2000" alt="Cover" class="w-full h-full object-cover opacity-20 mix-blend-overlay">
        </div>
        ...
        <div class="absolute top-16 md:top-24 left-4 md:left-6">
            <div class="w-28 h-28 md:w-36 md:h-36 bg-white rounded-full p-1 shadow-sm relative">
                <div class="w-full h-full bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center text-4xl md:text-5xl border-4 border-white shadow-inner">
                    🏆
                </div>
            ...
            <div class="flex justify-between items-start mt-2">
                <div class="max-w-2xl">
                    ...
                    <p class="text-[15px] md:text-base text-gray-900 mt-1 leading-snug">
                        Pusat Dokumentasi dan Publikasi Prestasi Siswa-Siswi | Membangun Generasi Unggul
                    </p>
                    ...
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-1 text-[13px] md:text-[14px] text-gray-500 mt-1.5">
                        ...
                        <a href="#" class="text-blue-600 font-semibold hover:underline hover:text-blue-800 transition">
                            {{ $prestasis->count() }} pencapaian diraih
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="flex flex-col gap-6 sm:gap-8">
        ...
        <div class="bg-white sm:rounded-2xl shadow-sm border-y sm:border border-gray-200 overflow-hidden">
            ...
            <div class="p-3 md:p-4 flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-yellow-400 to-orange-500 flex items-center justify-center text-white font-black border-2 border-white shadow-sm shrink-0">S</div>
                <div>
                    ...
                    <p class="text-[11px] text-gray-500">{{ \Carbon\Carbon::parse($prestasi->created_at)->diffForHumans() }}</p>
                </div>
            </div>

            @if($prestasi->gambar)
                <img src="{{ asset('storage/' . $prestasi->gambar) }}" alt="Prestasi" class="w-full h-auto max-h-[500px] object-cover border-y border-gray-100">
            @else
                <div class="w-full h-64 md:h-80 bg-gradient-to-br from-yellow-400 via-orange-500 to-red-500 flex flex-col items-center justify-center text-white border-y border-gray-100 relative overflow-hidden">
                    <span class="text-6xl md:text-7xl mb-2 drop-shadow-lg">🏆</span>
                    <h2 class="text-lg md:text-xl font-black px-6 text-center drop-shadow-md">{{ $prestasi->judul }}</h2>
                </div>
            @endif

            <div class="px-4 pt-3 pb-2 flex items-center gap-6 border-b border-gray-50">
                ...
                <form action="{{ route('like.toggle') }}" method="POST" class="m-0" onsubmit="toggleLike(event, this)">
                    ...
                    <button type="submit" class="flex items-center gap-1.5 transition active:scale-90 group {{ $isLiked ? 'text-pink-600' : 'text-gray-500 hover:text-pink-600' }}">
                        ...
                        <span class="text-[13px] font-medium">{{ $prestasi->likes ? $prestasi->likes->count() : 0 }}</span>
                    </button>
                </form>
                ...
                <div class="flex items-center gap-1.5 text-gray-500 group cursor-pointer">
                    ...
                    <span class="text-[13px] font-medium comment-count">{{ $prestasi->comments ? $prestasi->comments->count() : 0 }}</span>
                </div>
            </div>

            <div class="px-4 py-3">
                <p class="text-[14px] text-gray-900 leading-snug">
                    <span class="font-bold mr-1">Smecone Official</span>
                    Selamat kepada <span class="font-bold text-red-600">{{ $prestasi->nama_pemenang }}</span> atas pencapaian luar biasanya sebagai <span class="font-bold">{{ $prestasi->kategori_juara }}</span>! 🎉🔥
                </p>
                <h3 class="font-bold text-gray-900 text-sm mt-1">{{ $prestasi->judul }}</h3>
                <p class="text-[14px] text-gray-700 mt-1">{{ $prestasi->deskripsi }}</p>
            </div>
            ...
            <div class="px-4 pb-4">
                ...
                <div class="max-h-60 overflow-y-auto space-y-4 pr-2 hide-scrollbar comment-list">
                    @if($prestasi->comments && $prestasi->comments->count() > 0)
                        @foreach($prestasi->comments as $comment)
...
<script>
    // Tarik data avatar user yang lagi login buat dimunculin instan pakai Javascript
    ...
    async function submitComment(event, form) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Cek pakai foto profil atau pakai inisial
